 ...
 */
Network::Network()
    : _inputBuffer(1, 2000), _hidden1Buffer(1, 512), _hidden2Buffer(1, 256),
      _policyLogitsBuffer(1, 400), _valueOutBuffer(1, 1),
      _weightsShared1(2000, 512), _biasesShared1(1, 512),
      _weightsShared2(512, 256), _biasesShared2(1, 256),
      _weightsPolicy(256, 400), _biasesPolicy(1, 400), _weightsValue(256, 1),
      _biasesValue(1, 1) {
...
Output Network::predict(const Board &board) {
  ...
  auto my_threat = board.getWinningCandidates(1) | board.getThreatCandidates(1);
  auto op_threat = board.getWinningCandidates(2) | board.getThreatCandidates(2);
  ...
  for (int n = 0; n < 400; ++n) {
    ...
    inputData[n + 1200] = (my_threat.test(n)) ? 1.0f : 0.0f; // Fourth Channel
    inputData[n + 1600] = (op_threat.test(n)) ? 1.0f : 0.0f; // Fifth Channel
  }
  ...
  denseLayer(_hidden2Buffer, _weightsPolicy, _biasesPolicy,
             _policyLogitsBuffer);
